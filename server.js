...
app.post('/api/produits', async (req, res) => {
    const { nom, marque, ingredients, code_scanne } = req.body; // Supposons que ces champs sont envoyés dans le corps de la requête

    // Vérification de la présence des champs requis
    if (!nom || !marque || !ingredients || !code_scanne) {
        return res.status(400).json({ error: 'Données manquantes.' });
    }

    try {
        const result = await pool.query(
            'INSERT INTO produits (nom_produit, marque, ingredients, code_scanne) VALUES ($1, $2, $3, $4) RETURNING *',
            [nom, marque, ingredients, code_scanne]
        );
